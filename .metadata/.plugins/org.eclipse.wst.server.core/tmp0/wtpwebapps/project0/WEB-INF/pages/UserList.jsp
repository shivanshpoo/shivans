		...
		<div class="card card-table">
			...
			<sf:form method="post" modelAttribute="form">
				...
				<div  class="row p-2 d-flex justify-content-center">
					...
					<div class="col-lg-3">
						<sf:input path="lastName" cssClass="form-control border-left-right"
						...
						<table class="table table-hover table-striped border border-secondary">
							<thead>
								<tr>
									...
									<th scope="col" class="text-center"><s:message code="label.role"/></th>
							...
							<c:forEach items="${list}" var="user" varStatus="ct">
								<tr>
									<td class="text-center">
										<input class="case" name="chk_1" type="checkbox"  
                							<c:choose>
              								...
              								<c:if test="${user.roleId==1}">disabled</c:if>>
									</td>
									<td class="text-center">
									...
									<td class="text-center"><fmt:formatDate value="${user.dob}" type="date" pattern="dd-MM-yyyy"/></td>
									...
									<td class="text-center">
									<c:url var="editUrl" value="/ctl/User/User?id=" />
              							<c:choose>
              								<c:when test="${user.roleId != 1 && sessionScope.user.id != user.id}">
              									<a class="btn btn-outline-warning btn-block btn-round w-100" href="${editUrl}${user.id}"><i class="fa fa-trash" aria-hidden="true"></i><s:message code="label.edit"/></a>
              								</c:when>
              								<c:otherwise>
              									<button class="btn btn-outline-secondary btn-block btn-round w-100" disabled><i class="fa fa-plus" aria-hidden="true"></i><s:message code="label.edit"/></button>
              								</c:otherwise>
              							</c:choose>
									</td>
								</tr>
				...
				<c:if test="${empty list}">
					<div class="row d-flex justify-content-center">
						<div class="col-lg-2 col-6">
							<button type="submit" class="btn btn-outline-warning btn-block btn-round" name="operation" value="Back">
